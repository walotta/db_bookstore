...
class StoreInterface:
    ...
    def book_id_exist(self, store_id, book_id, session: Session) -> bool:
        match_book = (
    ...
    def store_id_exist(self, store_id, session: Session) -> bool:
        match_store = session.query(StoreSQL).filter_by(store_id=store_id).first()
        return match_store is not None

    def find_book(
        self, store_id: str, book_id: str, session: Session
    ) -> Optional[StoreBookTmp]:
        match_book = (
            session.query(StoreBookSQL)
            .filter_by(store_id=store_id, book_id=book_id)
            .first()
    ...
    def get_book_info(
        self, book_info_id: str, session: Session
    ...
    def add_stock_level(
        self, store_id: str, book_id: str, count: int, session: Session
    ) -> int:
        match_book = (
            session.query(StoreBookSQL)
            .filter_by(store_id=store_id, book_id=book_id)
    ...
    def get_store_seller_id(self, store_id: str, session: Session) -> Optional[str]:
        match_store = session.query(StoreSQL).filter_by(store_id=store_id).first()
    ...
    def insert_one_store(self, new_store: StoreTemp, session: Session) -> None:
